...
## Selection Rationale: Correctness, Performance, Reliability, and OS Constraints

| Decision | Correctness | Performance | Reliability and OS constraint |
| --- | --- | --- | --- |
| op-sqlite instead of AsyncStorage or expo-sqlite | SQLite gives indexed, transactional message lifecycle state and durable metadata such as retry counts, idempotency keys, server versions, and conflict fields. `expo-sqlite` was avoided because the assignment explicitly requires `@op-engineering/op-sqlite`. | Queue-critical writes use synchronous calls so the app can persist before returning UI success. The write surface is intentionally small and bounded to avoid long JS stalls. | WAL plus `PRAGMA synchronous = FULL` prioritizes crash durability. Multi-step writes use transactions so app/process death does not leave partially updated queue state. |
| JS sync processor with native scheduling | Keeping queue logic in TypeScript avoids divergent Android/iOS business rules and lets tests cover one shared processor. | Batches are bounded (`batchSize`, `maxBatchesPerRun`) so a background wake-up does not monopolize the JS thread. UI send remains local-first; network sync runs after persistence. | The module-level mutex prevents double-send behavior. The mutex is released in `finally`, retries use backoff, and circuit breaker state prevents repeated failing sync runs. |
| Android WorkManager plus BackgroundFetch | WorkManager gives the Android side a native, persisted scheduler instead of relying only on JS timers. | Scheduling happens natively. The actual sync path is still shared with JS to keep logic consistent, and the work is bounded to reduce wake-up cost. | Android Doze, app standby buckets, battery saver, and OEM restrictions can delay work. WorkManager with network/battery constraints is the correct durable path, but it is not a real-time delivery guarantee. |
| iOS BGTaskScheduler plus BackgroundFetch | iOS background refresh and processing identifiers are registered in native code, with the same sync processor used when JS is invoked. | iOS gives limited execution windows, so the sync processor is intentionally partial-progress safe and batch-limited. | iOS does not run background fetch or BGTaskScheduler tasks after the user force-quits the app. Background work resumes only after the user opens the app again. |
| Hermes over JSC | `android/gradle.properties` has `hermesEnabled=true`; the Android build includes `hermes-android` when enabled. | Hermes usually improves React Native startup and memory behavior through bytecode execution and tighter integration with RN. | JSC remains defined as a fallback in Gradle, but Hermes is the selected runtime for this submission because it matches modern RN defaults and reduces Android startup risk. |
| New Architecture and Fabric enabled | `newArchEnabled=true` keeps the project aligned with React Native 0.85 defaults and current native module expectations. | Fabric can reduce rendering overhead for supported components and keeps the app on the modern RN path. | This repo does not add custom Fabric components or TurboModules. Native dependencies are limited to well-known libraries that support the current RN architecture. |
| FlashList for chat | FlashList is used for the 10,000-message requirement and includes the assignment-required `estimatedItemSize` prop in the selected v1.x line. | Virtualization avoids retaining/rendering all 10,000 rows at once. The hook pages from SQLite so React state holds only the loaded page window. | This keeps memory and scroll performance predictable on mobile devices, especially after seeding large local histories. |
| react-native-video for login | The assignment requires a video-backed login screen. The screen uses a poster and local motion layer so it is not blank if video decode is delayed. | Native ExoPlayer/AVPlayer handles decode off the JS thread. The buffer config is intentionally small to avoid large startup memory spikes. | The video pauses on app background and is released on unmount by React Native lifecycle cleanup. |
| NetInfo for network state | NetInfo exposes OS-level connectivity state for the offline banner and sync decisions instead of custom ping polling. | Native connectivity events avoid wasteful JS polling. | Connectivity can be unknown at startup, so sync correctness still depends on retry/backoff and durable queue state rather than trusting one network snapshot. |
